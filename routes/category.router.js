...
router.post("/worker/create", isNews, async (req, res) => {
    await Category.create({
        name_tm: req.body.name_tm,
        name_en: req.body.name_en,
        name_ru: req.body.name_ru,
...
router.get("/worker/edit/:categoryId", isNews, async (req, res) => {
    await Category.findOne({ where: { id: req.params.categoryId } }).then((category) => {
        res.json({ category: category })
    })
});

router.post("/worker/edit/:categoryId", isNews, async (req, res) => {
    await Category.update({
        name_tm: req.body.name_tm,
        name_en: req.body.name_en,
        name_ru: req.body.name_ru,
...
router.delete("/worker/delete/:categoryId", isNews, async (req, res) => {
    await Category.findOne({ where: { id: req.params.categoryId } }).then((category) => {
        if (category) {
            category.destroy()
            return res.json({ success: "Üstünlikli pozuldy" })
